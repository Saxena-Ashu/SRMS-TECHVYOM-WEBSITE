...
app.post('/register', (req, res) => {
  const { name, phoneno, emailid, rollno, aadhaar, college, course, year } = req.body;
  const selectedEvents = Array.isArray(req.body.events) ? req.body.events : [req.body.events].filter(Boolean);

  const checkQuery = `SELECT * FROM registrations WHERE aadhaar = ? OR emailid = ?`;
  connection.query(checkQuery, [aadhaar, emailid], (err, results) => {
    ...
    connection.query(getLastIdQuery, (err, result) => {
      ...
      const pid = `TV2025-${String(newId).padStart(4, '0')}`;

      const insertQuery = `
        INSERT INTO registrations (pid, name, phoneno, emailid, rollno, aadhaar, college, course, year)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;

      connection.query(
        insertQuery,
        [pid, name, phoneno, emailid, rollno, aadhaar, college, course, year],
        (err, result) => {
          if (err) return res.redirect('/register.html?status=error');
...
app.post('/register-team', (req, res) => {
  ...
  connection.query(findRegistrationsQuery, [pids], (err, registrations) => {
    ...
    connection.query(getLastTeamIdQuery, (err, result) => {
      ...
      const lastId = result.length > 0 ? result[0].id : 0;
      const newId = lastId + 1;
      const team_id = `TEAM-${String(newId).padStart(4, '0')}`;
...
app.get('/results', (req, res) => {
  ...
  const searchSQL = `
    ...
    WHERE r.name LIKE ? OR r.phoneno LIKE ? OR r.emailid LIKE ? OR r.rollno LIKE ? OR r.college LIKE ?
    GROUP BY r.id`;
...
app.get('/api/event-lists', (req, res) => {
  ...
  if (type === 'team') {
    ...
    connection.query(query, [events], (err, results) => {
      if (err) {
        console.error('Error fetching team event lists:', err);
        return res.status(500).json({ error: 'Database error' });
      }
      ...
      events.forEach(event => {
        formattedData[event] = results.filter(row => row.event_name === event).map(row => ({
          ...
          members: row.members ? row.members.split(', ').map(m => {
            const match = m.match(/(.*)\((.*)\)/);
            return match ? { name: match[1].trim(), pid: match[2].trim() } : { name: m.trim(), pid: '' };
          }) : []
        }));
      });
...
app.put('/api/registration/:id', (req, res) => {
  const id = req.params.id;
  const { name, phoneno, emailid, rollno, aadhaar, college, course, year, events } = req.body;
  ...
  connection.beginTransaction((err) => {
    ...
    const updateQuery = `
      ...
      SET name = ?, phoneno = ?, emailid = ?, rollno = ?, aadhaar = ?, college = ?, course = ?, year = ?
      WHERE id = ?`;

    connection.query(
      updateQuery,
      [name, phoneno, emailid, rollno, aadhaar, college, course, year, id],
      (err) => {
        if (err) {
...
app.put('/api/team/:id', (req, res) => {
  const id = req.params.id;
  const { team_name, events } = req.body;
  ...
  connection.beginTransaction((err) => {
    ...
    connection.query(updateTeamQuery, [team_name, id], (err) => {
      ...
      // Delete existing team events
      const deleteTeamEventsQuery = `DELETE FROM team_events WHERE team_id = ?`;
      connection.query(deleteTeamEventsQuery, [id], (err) => {
        if (err) {
          console.error('Error deleting existing team events:', err);
          return connection.rollback(() => {
            res.status(500).json({ error: 'Database error' });
          });
        }
        ...
        if (events && events.length > 0) {
          ...
          connection.query(insertEventsQuery, [eventValues], (err) => {
            if (err) {
              console.error('Error inserting updated team events:', err);
              return connection.rollback(() => {
                res.status(500).json({ error: 'Database error' });
              });
            }
          ...
          // No events to insert, just commit the transaction
          connection.commit((err) => {
            if (err) {
              console.error('Error committing transaction:', err);
              return connection.rollback(() => {
