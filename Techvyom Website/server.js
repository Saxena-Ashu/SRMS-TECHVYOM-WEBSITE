...
app.delete('/api/registration/:id', async (req, res) => {
  ...
  try {
    ...
    await mongoose.connection.transaction(async session => {
      ...
      const teamMemberships = await TeamMember.find({ registration_id: id }).session(session);
      if (teamMemberships.length > 0) {
        await TeamMember.deleteMany({ registration_id: id }).session(session);
        
        // For each team, check if it's now empty and delete if needed
        ...
        for (const teamId of teamIds) {
          ...
          if (remainingMembers === 0) {
            await TeamEvent.deleteMany({ team_id: teamId }).session(session);
      ...
      const result = await Registration.findByIdAndDelete(id).session(session);
    ...
    res.status(500).json({ 
      success: false,
      message: err.message || 'Failed to delete registration',
      error: err.message
    });
  }
});
