...
app.get('/api/team/:id', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...team.toObject(),
      members: members.filter(m => m.registration_id).map(m => m.registration_id),
      college: members.length > 0 && members[0].registration_id ? members[0].registration_id.college : '',
      events: events.map(e => e.event_name)
    });
  } catch (err) {
